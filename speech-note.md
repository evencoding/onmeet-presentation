...
제 개인 회고입니다. 5개 영역에서의 도전과 해결 경험을 정리했습니다.

**상태 관리 전략** — 가장 고통스러웠던 문제가 **STT 리렌더 폭풍**이었습니다. 실시간으로 STT 텍스트가 들어오면 상태가 밀리초 단위로 갱신되면서 화면 전체가 리렌더되는 현상이 발생했습니다. useRef 버퍼에 텍스트를 누적하다가 Throttle 간격마다 스냅샷을 찍어 렌더링하는 **Buffer → Throttle → Snapshot** 패턴으로 해결했고, 최종적으로 서버 상태(TanStack Query), 클라이언트 상태(Zustand), 일시 상태(useRef) 3개의 저장소를 명확히 분리하는 체계를 완성했습니다.

**SSE 파서 직접 구현** — EventSource API가 커스텀 헤더를 지원하지 않는 한계 때문에, fetch + ReadableStream으로 SSE 프로토콜을 직접 파싱하는 커스텀 파서를 만들었습니다. 5초 자동 재연결 로직도 포함시켰고, 이걸 통해 인증 헤더가 필요한 SSE 연결을 안전하게 유지할 수 있게 되었습니다.

**디자인 패턴** — VAC 패턴으로 597줄을 225줄로 줄인 경험, Custom Hooks 추출, Compound Components 적용으로 관심사 분리와 코드 축소를 동시에 달성했습니다.

**빌드 & 성능** — Manual Chunk Splitting으로 대형 라이브러리를 독립 청크화하고, console 자동 제거, Route Prefetching으로 즉각적 페이지 전환을 구현했습니다.

**성장 & 교훈** — Promise 싱글턴 패턴으로 동시다발 401 응답에 대한 토큰 갱신을 한 번만 수행하는 로직을 구현했고, 253개 테스트 케이스를 작성했습니다. 가장 큰 교훈은 **"왜" 답할 수 있는 힘**과 **도구의 경계를 정하는 능력**이었습니다.
